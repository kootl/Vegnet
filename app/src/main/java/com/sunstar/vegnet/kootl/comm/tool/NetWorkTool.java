...
public class NetWorkTool {
    ...
    public static boolean isNetWorkConnected() {
        Context context = KooApplication.getAppContext();
     ...
     * 推荐小于请求网络工具的超时时间
     */
    private static final boolean isCanRealAccessInternet() {
     ...
     * 推荐小于请求网络工具的超时时间
     */
    private static final boolean isCanRealAccessInternetTwo() {
        if (!isNetWorkConnected()) {
            return false;
        }
        /**
    ...
    @Deprecated //暂时不用
    public static void checkInternetState(final OnInternetCallBack onInternetCallBack) {
        KLog.d("checkInternetState start");
        new Thread(new Runnable() {
            @Override
            public void run() {
                final boolean isCan = isCanRealAccessInternet();
