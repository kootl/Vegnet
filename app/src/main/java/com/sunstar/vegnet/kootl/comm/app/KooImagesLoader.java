...
import com.sunstar.vegnet.kootl.comm.tool.ReflectTool;
...
public class KooImagesLoader {
    ...
    public void loadImage(final String path, final ImageView imageView, int width, int height) {
        ImageSize imageSize = new ImageSize();
        imageSize.width = width;
        imageSize.height = height;
        loadImageBase(path, imageView, imageSize);
    }

    public void loadImage(final String path, final ImageView imageView) {
        loadImageBase(path, imageView, null);
    }
     ...
     * @param imageView
     */
    private void loadImageBase(final String path, final ImageView imageView, ImageSize imageSize) {
         ...
         *内存
         */
        Bitmap bitmap = LruCacheBitmapTool.getCacheFromMemory(path);
        if (bitmap != null) {
            //内存中有  直接显示
             ...
             *  //内存中没有 加入队列
             */
            this.addRunnableTask(buildRunnableTask(path, imageView, imageSize));
        }
     ...
     * @return
     */
    private Runnable buildRunnableTask(final String pathOrUrl, final ImageView imageView, final ImageSize imageSize) {
        ...
        final Runnable runnable = new Runnable() {
            ...
            public void run() {
                Bitmap bitmap;
                if (RegexTool.checkURL(pathOrUrl)) {
                    KLog.d("网络图片" + pathOrUrl);
                    //网络图片
                    bitmap = LruCacheBitmapTool.getCacheFromDisk(pathOrUrl);
                    if (bitmap == null) {
                        //硬盘里也没有缓存

                        ImageSize imageSizeTemp = imageSize;
                        if (imageSizeTemp == null) {
                            imageSizeTemp = getWishImageSize(imageView);
                        }
                        int reqWidth = imageSizeTemp.width;
                        int reqHeight = imageSizeTemp.height;
                        //
                        bitmap = ImageTool.downloadImageToBitmapWishImageSize(pathOrUrl, reqWidth, reqHeight);
                        //
                        LruCacheBitmapTool.saveCacheToDisk(pathOrUrl, bitmap);
                    }
                ...
                // 把图片加入到内存缓存
                LruCacheBitmapTool.saveCacheToMemory(pathOrUrl, bitmap);
    ...
    private Bitmap zoomBitmapFromPath(final String path, final ImageView imageView) {
        ...
        // 2、压缩图片
        Bitmap bitmap = ImageTool.decodeSampledBitmapFromPath(path, reqWidth,
                reqHeight);
    ...
    private ImageSize getWishImageSize(ImageView imageView) {
        ImageSize imageSize = new ImageSize();
        int screenWidth = ScreenTool.getScreenWidth(imageView.getContext());
        int screenHeight = ScreenTool.getScreenHeight(imageView.getContext());
        ...
        if (width <= 0) {
            ...
            try {
                width = ReflectTool.getFieldValue(imageView, "mMaxWidth");
            } catch (Exception e) {
                //  e.printStackTrace();
                KLog.w(e.getMessage());
            }
//            width = ReflectUtil.getImageViewField(imageView, "mMaxWidth");
        ...
        if (height <= 0) {
            try {
                height = ReflectTool.getFieldValue(imageView, "mMaxHeight");// 检查最大值
            } catch (Exception e) {
                e.printStackTrace();
            }
