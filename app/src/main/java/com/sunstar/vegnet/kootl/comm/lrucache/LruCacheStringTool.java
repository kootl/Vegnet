package com.sunstar.vegnet.kootl.comm.lrucache;
...
import com.sunstar.vegnet.kootl.comm.tool.SdcardTool;
...
public class LruCacheStringTool {
    private static final String TAG = "LruCacheStringTool";
    ...
    private static void initDiskLruCache(Context context) {
        try {
            File diskCacheDir = new File(SdcardTool.getDiskCacheDirPath(context) + File.separator + DISKLRUCACHE_STRING_UNIQUE_NAME);
    ...
    public static String getCacheFromMemory(String keyRaw) {
        return mLruCache.get(MD5Tool.encode(keyRaw));
    }

    private static void saveCacheToMemory(String keyRaw, String value) {
        if (getCacheFromMemory(MD5Tool.encode(keyRaw)) == null) {
            //不存在时添加
            mLruCache.put(MD5Tool.encode(keyRaw), value);
        }
    }
    ...
    public static String getCacheFromDiskAndSaveCacheToMemory(String keyRaw) {
        String tempStr = null;
        try {
            DiskLruCache.Snapshot snapshot = mDiskLruCache.get(MD5Tool.encode(keyRaw));
            if (snapshot != null) {
 ...
 */
    ...
    public static void saveCacheToDisk(final String keyRaw, final String needSaveString) {
        ...
        try {
            editor = mDiskLruCache.edit(MD5Tool.encode(keyRaw));
            if (editor != null) {
    ...
    public static void removeDiskCache(String keyRaw) {
        try {
            if (mDiskLruCache != null) {
                mDiskLruCache.remove(MD5Tool.encode(keyRaw));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
