...
public class LruCacheBitmapTool {
    private static final String TAG = "LruCacheBitmapTool";
    ...
    public static Bitmap getCacheFromMemory(String keyRaw){
        Bitmap bitmap=mLruCache.get(MD5Tool.encode(keyRaw));
      return bitmap;
    }
    public static void saveCacheToMemory(String keyRaw, Bitmap bitmap){
        ...
        if (getCacheFromMemory(keyRaw) == null) {
            //不存在时添加
            mLruCache.put(MD5Tool.encode(keyRaw), bitmap);
        }
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    editor = mDiskLruCache.edit(MD5Tool.encode(urlStr));
                    if (editor!=null) {
